...
helpers.disconnect = (firewall, username, computer, key, callback) => {
    var firewall = typeof('string') && firewall.length > 0 ? firewall : false;
    var key = typeof('string') && key.length > 0 ? key : false;
    var username = typeof('string') && username.length > 0 ? username : false;
    ...
    if(firewall && key && username && computer){
        ...
        var cmd = '<request><global-protect-gateway><client-logout><domain>multi</domain><gateway>GP_GATEWAY_CLIENT-N</gateway><user>'+username+'</user><reason>force-logout</reason><computer>'+computer+'</computer></client-logout></global-protect-gateway></request>'
        cmd = querystring.escape(cmd)
